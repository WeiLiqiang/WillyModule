...
class NewProjectBusinessRepository : BaseBusinessRepository() {

    private val apiService by lazy { SystemRetrofitClient.service }

    suspend fun getNewProjectList(currentPage: Int): HttpResult<ApiPageResponse<Article>> {
    ...
    private suspend fun requestArticleList(page: Int): HttpResult<ApiPageResponse<Article>> =
        executeResponse(apiService.getNewProjectList(page))
}