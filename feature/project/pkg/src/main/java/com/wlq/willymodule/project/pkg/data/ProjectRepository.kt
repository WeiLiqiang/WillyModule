...
class ProjectRepository : BaseBusinessRepository() {
    ...
    suspend fun getProjectTypeList(): HttpResult<ApiPageResponse<SystemParent>> =
        safeApiCall(call = { executeResponse(apiService.getProjectTypeList()) })


    suspend fun getProjectDetailList(page: Int, cid: Int): HttpResult<ApiPageResponse<Article>> =
