package com.wlq.willymodule.base.business.camera.manager.impl
...
import com.wlq.willymodule.base.business.camera.listener.CameraCloseListener
import com.wlq.willymodule.base.business.camera.listener.CameraOpenListener
import com.wlq.willymodule.base.business.camera.listener.CameraPhotoListener
import com.wlq.willymodule.base.business.camera.listener.CameraVideoListener
...
import com.wlq.willymodule.base.business.camera.util.XLog.e
import com.wlq.willymodule.base.business.camera.util.XLog.i
...
class Camera1Manager(cameraPreview: CameraPreview) : BaseCameraManager<Int?>(cameraPreview) {
        ...
        set(displayOrientation) {
            ...
            if (isCameraOpened) {
                ...
                camera?.setDisplayOrientation(
                    CameraHelper.calDisplayOrientation(
                        context, cameraFace,
    ...
    private fun prepareCameraOutputs() {
        try {
            ...
            d(
                "Camera1Manager",
                "prepareCameraOutputs cost : " + (System.currentTimeMillis() - start) + " ms"
            )
        } catch (ex: Exception) {
    ...
    private fun adjustCameraParameters(
        forceCalculateSizes: Boolean,
        changeFocusMode: Boolean,
        changeFlashMode: Boolean
    ) {
        ...
        d(
            "Camera1Manager",
            "adjustCameraParameters size cost : " + (System.currentTimeMillis() - start) + " ms"
        )
        ...
        d(
            "Camera1Manager",
            "adjustCameraParameters focus cost : " + (System.currentTimeMillis() - start) + " ms"
        )
        start = System.currentTimeMillis()
        if (changeFlashMode) {
        ...
        d(
            "Camera1Manager",
            "adjustCameraParameters flash cost : " + (System.currentTimeMillis() - start) + " ms"
        )
        start = System.currentTimeMillis()
        setZoomInternal(parameters)
        d(
            "Camera1Manager",
            "adjustCameraParameters zoom cost : " + (System.currentTimeMillis() - start) + " ms"
        )
        start = System.currentTimeMillis()
        if (showingPreview) {
            showingPreview = false
        ...
        d(
            "Camera1Manager",
            "adjustCameraParameters restart preview cost : " + (System.currentTimeMillis() - start) + " ms"
        )
    }
    ...
    private fun prepareVideoRecorder(): Boolean {
        ...
        try {
            ...
            if (camcorderProfile != null) {
                ...
                e(
                    "Camera1Manager",
                    "Failed to initialize video recorder, the camcorder profile was null."
                )
            }
    ...
    private fun setFlashModeInternal(parameters: Parameters?) {
        ...
        try {
            when (flashMode) {
                ...
                FlashMode.FLASH_AUTO -> if (modes.contains(Parameters.FLASH_MODE_AUTO)) {
                    params.flashMode = Parameters.FLASH_MODE_AUTO
                }
                else -> if (modes.contains(Parameters.FLASH_MODE_AUTO)) {
                    params.flashMode = Parameters.FLASH_MODE_AUTO
                }
            }
            if (nullParameters) {
    ...
    private fun setFlashModeOrAuto(
        parameters: Parameters?,
        supportModes: List<String>,
        mode: String
    ) {
    ...
    private fun setZoomInternal(parameters: Parameters?) {
        ...
        if (params.isZoomSupported) {
            params.zoom = CameraHelper.getZoomIdxForZoomFactor(params.zoomRatios, zoom)
            if (nullParameters) {
                camera?.parameters = params
            }
        }
